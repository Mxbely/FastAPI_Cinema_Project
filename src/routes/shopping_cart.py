...
from schemas.shoping_cart import CartCreate, CartResponse, CartItemResponse, CartItemDetail
from database.models.accounts import User
from security.auth import get_current_user
from validation.shoping_cart import validate_not_in_cart, validate_not_purchased, validate_movie_availability
from database.crud.shopping_cart import get_user_cart, get_movie_by_id, get_cart_item, create_cart, add_cart_item, delete_cart_item
...
def add_to_cart(cart_data: CartCreate, db: Session = Depends(get_postgresql_db), user: User = Depends(get_current_user)):
...
def get_cart(db: Session = Depends(get_postgresql_db), user: User = Depends(get_current_user)):
    cart = get_user_cart(user, db)
    if not cart:
...
def remove_from_cart(movie_id: int, db: Session = Depends(get_postgresql_db), user: User = Depends(get_current_user)):
    cart = get_user_cart(user, db)
    if not cart:
